...
4. **Never invent a design token.** Colors, spacing, radii, and shadows come from
   `docs/DESIGN.md`. If you need a value that isn't there, say so rather than
   picking one.
5. **A decision that outlives the session goes in `docs/DECISIONS.md`.** Append only.
   Never rewrite a past decision — supersede it with a new numbered entry.
6. **Report at the end of every session** in this shape:
...
Front-End/src/
  ...
  content/         [NEW] Arabic display layer — roles.ts, ui.ts
...
packages/shared/   types, socket event names, constants
docs/              this plan
```
